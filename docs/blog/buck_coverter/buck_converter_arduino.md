...
## Coursework
You need to prepare a report of the experiment with the DC-DC buck converter.  The voltage of following important points are required to be present in your report for each question:
- Output voltage
- PWM signal generated by Arduino
- Gate voltage of the MOSFET

The signals should be captured for 25%, 60% and 100% duty cycles.

Q1- Capture the mentioned signals when there is no flywheel circuit.

Q2- Capture the mentioned signals when there is no flywheel circuit, but only a 100uF capacitor at the load.

Q3- Capture the mentioned signals when there is the flywheel circuit with no extra capacitors.

Q4- Analyze and compare the results of the previous three questions from the following aspects:

- How each change in the circuit affected the output voltage.
- How the duty cycle affected the ripple voltage on the output.
